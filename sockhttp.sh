...
echo -e "#       Setup By: Natzkie                          #"
echo -e "#       Server System: Certified tambay            #"
echo -e "#        owner: Mcking                             #"
echo -e "#############################################${RESET}"
}
clear
...
screen -dmS proxy python /etc/SSHPlus/proxy.py 8080
sed -i '/proxy.py/d' /etc/autostart
echo -e "netstat -tlpn | grep -w 8080 > /dev/null || screen -dmS proxy python /etc/SSHPlus/proxy.py 8080" >>/etc/autostart

/bin/cat <<"EOM" >/etc/autostart
#!/bin/bash
clear
#INICIO AUTOMATICO
netstat -tlpn | grep -w 8080 > /dev/null || screen -dmS proxy python /etc/SSHPlus/proxy.py 8080
EOM
...
BUFLEN = 8196 * 8
TIMEOUT = 60
MSG = 'GTM NO LOAD Powered by: PinoyVPN'
...
(crontab -l 2>/dev/null || true; echo "*/5 * * * * /bin/bash /root/cron.sh >/dev/null 2>&1
